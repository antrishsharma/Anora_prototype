...
SYSTEM_PROMPT = """You are Anora, a smart AI assistant created by Antrish Sharma. You can control the PC. Keep answers short and conversational. Always reply in English only, even if the user speaks Hindi, Hinglish, or any other language. Never use Hindi words or Devanagari script in your replies — English only, every time."""
...
def play_youtube(query):
    # pywhatkit automatically searches YouTube and plays the first result
    kit.playonyt(query)
...
def detect_command(text):
    ...
    if any(word in text for word in ["chrome kholo", "open chrome", "chrome open"]):
        ...
        song = song.replace("on youtube", "").replace("youtube", "").strip()
        if song:
            play_youtube(song)
            return f"Playing {song} on YouTube!"
        return "Kya play karna hai, naam bata?"
    ...
    elif any(phrase in text for phrase in ["volume up", "volume badha", "louder", "awaaz badha"]) or ("volume" in text and any(w in text for w in ["up", "increase", "badha", "high", "max"])):
        volume_up()
        return "Volume badha diya!"
    elif any(phrase in text for phrase in ["volume down", "volume kam", "quieter", "awaaz kam"]) or ("volume" in text and any(w in text for w in ["down", "decrease", "kam", "low", "halki"])):
        volume_down()
